...
int main(int argc, char **argv) {
	...
	if(rank == 0) {
		...
		// Store the image sizes in the buffer
		image_parameters[0] = image.x;
		image_parameters[1] = image.y;
		image_parameters[2] = image.depth;

	}

	// Broadcast the image sizes to the other ranks
	MPI_Bcast(image_parameters,	// buffer of image sizes
		3,			// length of array
		MPI_INT,		// type
		0,			// root source
		MPI_COMM_WORLD);	// broadcast to world

	// Malloc image.pixels for all other ranks
	if(rank != 0){
		image.pixels = malloc(image_parameters[0]*image_parameters[1]*image_parameters[2]*sizeof(char));
	}

	// Make image parameters available to all other ranks
	image.x = image_parameters[0];
	image.y = image_parameters[1];
	image.depth = image_parameters[2];
	
	// Broadcast the entire image data from rank0 to other ranks
	MPI_Bcast(image.pixels,
		image_parameters[0]*image_parameters[1]*image_parameters[2],
		MPI_CHAR,
		0,
		MPI_COMM_WORLD);
